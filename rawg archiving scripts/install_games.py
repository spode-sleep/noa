...
# gogdl (GOG) — конфигурация
GOGDL_AUTH_CONFIG = Path.home() / ".noa" / "gog_auth.json"
GOG_AUTH_URL = (
...
def _gogdl_get_token() -> str | None:
    """Получить свежий GOG access_token через gogdl auth (с автообновлением)."""
    try:
        result = subprocess.run(
...
def auth_gogdl() -> bool:
    ...
    При первом запуске открывает браузер для OAuth-авторизации.
    Токены хранятся в ~/.noa/gog_auth.json.
    """
    ...
    try:
        ...
        log("[gogdl] Браузер открыт автоматически")
    except Exception:
        pass
    ...
    try:
        result = subprocess.run(
            [
                "gogdl", "--auth-config-path", str(GOGDL_AUTH_CONFIG),
                "auth", "--code", code,
            ],
        ...
        if result.returncode == 0 and result.stdout.strip():
            data = json.loads(result.stdout)
            if data and data.get("access_token"):
                log("[gogdl] ✓ Авторизация GOG завершена")
                return True
    except (FileNotFoundError, subprocess.TimeoutExpired, json.JSONDecodeError):
        pass

    err("[gogdl] ✗ Не удалось авторизоваться в GOG")
    return False
...
def _gog_search_owned(game_name: str, token: str) -> tuple[int, str] | None:
    """Поиск игры в библиотеке GOG по имени через GOG API.

    Возвращает (product_id, title) или None.
    """
    encoded = urllib.parse.quote(game_name)
    url = f"https://embed.gog.com/account/getFilteredProducts?mediaType=1&search={encoded}"
    ...
    for product in products:
        ...
        if title.lower().strip() == norm_search:
            return (product["id"], title)

    # Нечёткое: поисковый запрос содержится в названии
    for product in products:
        title = product.get("title", "")
        if norm_search in title.lower():
            return (product["id"], title)

    return None


def try_gogdl(game_name: str, out_dir: Path, log_file: Path) -> bool:
    """Попытка скачать через gogdl (GOG.com).
    ...
    token = _gogdl_get_token()
    if not token:
        warn("  [gogdl] Не удалось получить токен")
        return False

    match = _gog_search_owned(game_name, token)
    if not match:
        warn(f"  [gogdl] «{game_name}» не найдена в библиотеке")
        return False
    ...
    try:
        ...
        err("  [gogdl] Таймаут скачивания (2 часа)")
        return False
    except FileNotFoundError:
