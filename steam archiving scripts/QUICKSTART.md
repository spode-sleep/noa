...
## 🔧 УСТАНОВКА IONICE (ОДИН РАЗ):

`ionice` снижает приоритет дисковых операций, чтобы скачивание и копирование
не мешали работе системы. Входит в пакет `util-linux` (обычно уже установлен):
...
Скрипт автоматически определяет наличие `ionice`. Если его нет — всё
работает и без него, просто без снижения приоритета I/O.
...
- [ ] ionice установлен (`command -v ionice`, опционально)
