...
├─ 0. Проверяет — если игра уже на HDD, пропускает
...
```bash
GAME_LANG="russian"                             # Язык (english, german, и т.д.)
...
✅ **Верификация копирования** - сравнение размеров перед удалением локальной копии
✅ **Пропуск установленных** - уже скачанные на HDD игры не скачиваются повторно
✅ **Очистка при прерывании** - Ctrl+C корректно удаляет частичные загрузки и временные файлы
✅ **Защита учётных данных** - временный файл с паролем доступен только владельцу
✅ **Проверка доступности HDD** - предупреждение если диск отключился
