...
export async function runAgent(
  ...
  const systemPrompt = [
    ...
    ``,
    `The file contents below are ALREADY LOADED — do NOT re-read them. Go straight to making changes.`,
    ``,
    `WORKFLOW (follow in order):`,
    `1. IMPLEMENT: Make changes immediately (str_replace for targeted edits, write/create for new files)`,
    `2. VERIFY: Check with "git status" and run tests/builds if applicable`,
    `3. COMMIT & PUSH: Run "git add -A && git commit -m 'description' && git push" — MANDATORY`,
    ``,
    `IMPORTANT RULES:`,
